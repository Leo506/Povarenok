...
if (context.PickupPoints.Any() is false)
{
    ...
    {
        await context.PickupPoints.AddAsync(new DemoExam.Core.Models.PickupPoint()
        {
            PostIndex = data[0],
            City = data[1],
            Street = data[2]
...
if (context.Orders.Any() is false)
{
    ...
    {
        await context.Orders.AddAsync(new DemoExam.Core.Models.Order()
        {
...
if (context.Products.Any() is false)
{
    ...
    {
        await context.Products.AddAsync(new DemoExam.Core.Models.Product()
        {
...
if (context.OrderLists.Any() is false)
{
    ...
    {
        await context.OrderLists.AddAsync(new DemoExam.Core.Models.OrderList()
        {
...
if (context.Roles.Any() is false)
{
    ...
    {
        await context.Roles.AddAsync(new DemoExam.Core.Models.Role() { RoleName = item });
    }
...
if (context.Users.Any() is false)
{
    await ImportData("Data/User.csv", ';', async data =>
    {
        ...
        await context.Users.AddAsync(new DemoExam.Core.Models.User()
        {
            UserSurname = data[0],
...
{
    foreach (var item in await File.ReadAllLinesAsync(path))
    {
        await actionForAdd(item.Split(delimeter));
    }
    await context.SaveChangesAsync();
}
